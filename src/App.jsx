...
export default function App() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #2D0845 0%, #7B2D8B 50%, #C0395A 100%)' }}>
      ...
      <div style={{ fontSize: '20px', fontWeight: '800', color: '#fff', letterSpacing: '3px' }}>BeautySureAQL</div>
      <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.6)', marginTop: '6px' }}>Loading platform…</div>
    </div>
  )
  ...
  if (!user.email_confirmed_at && !user.confirmed_at) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #2D0845, #7B2D8B, #C0395A)', padding: '20px' }}>
      <div style={{ background: '#fff', borderRadius: '20px', padding: '36px', maxWidth: '400px', textAlign: 'center' }}>
        <div style={{ fontSize: '44px', marginBottom: '12px' }}>📧</div>
        <div style={{ fontSize: '18px', fontWeight: '700', color: '#7B2D8B', marginBottom: '8px' }}>Confirm Your Email</div>
        <div style={{ fontSize: '13px', color: '#6B5878', marginBottom: '20px', lineHeight: '1.6' }}>
          Please click the confirmation link sent to <strong>{user.email}</strong> before logging in.
        </div>
  ...
  // ── Company admin confirmed but company link missing ──────
