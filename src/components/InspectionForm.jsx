...
export default function InspectionForm({ inspectionId, profile, onClose }) {
  ...
  async function saveDraft() {
    ...
    setSaving(true)
    const payload = { ...form, batch_size: form.batch_size ? Number(form.batch_size) : null, defect_counts: counts, status: 'draft', created_by: profile.id, updated_at: new Date().toISOString() }
    let error
    if (existingId) {
