...
export default function AdminSignup({ onBack }) {
  ...
  const [form, setForm]     = useState({
    ...
    company_name: '', company_country: '', phone: '', message: '',
  })
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true)
    try {
      // 1. Create Supabase auth user (role=company_admin triggers pending_approval in DB)
      const { data: authData, error: authErr } = await supabase.auth.signUp({
        email:    form.email,
        password: form.password,
        options:  { data: { full_name: form.full_name, role: 'company_admin' } },
      })
      ...
      // 3. Trigger approval email Edge Function
      ...
      <div style={s.card}>
        <div style={{ textAlign: 'center', padding: '16px 0' }}>
          ...
          <div style={{ fontSize: '13px', color: '#6B5878', lineHeight: '1.7', marginBottom: '20px' }}>
            Your company admin request has been sent for approval.<br />
            You will receive an email at <strong>{form.email}</strong> once approved.<br /><br />
            <span style={{ fontSize: '11px', color: '#A494B0' }}>
              Approvals are typically reviewed within 24 hours.<br />
              For urgent queries: <a href="mailto:sainiboyynr@zohomail.in" style={{ color: '#7B2D8B' }}>sainiboyynr@zohomail.in</a>
            </span>
          </div>
          <div style={s.pillRow}>
            <span style={s.pill}>✅ Account Created</span>
            <span style={{ ...s.pill, background: '#FEF3E2', color: '#7A4A00' }}>⏳ Awaiting Approval</span>
          </div>
    ...
    <div style={s.wrap}>
      ...
      <div style={s.card}>
        <div style={s.header}>
          ...
          <div style={s.brandSub}>Admin Account · Requires Approval</div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={s.group}>
            ...
            <textarea style={{ ...s.input, height: '64px', resize: 'vertical' }}
              placeholder="Brief description of your company or any specific requirements…"
        ...
        <div style={s.note}>
          After submission, your request will be reviewed by the platform administrator.
